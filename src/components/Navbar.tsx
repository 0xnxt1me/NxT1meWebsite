...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className="fixed w-full bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm z-10 py-4">
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-white dark:bg-slate-900 px-4 pt-4 pb-6 shadow-md">
          <div className="flex flex-col items-start space-y-4">
            <Link
              to="/about"
              onClick={() => setMobileMenuOpen(false)}
              className="text-slate-700 dark:text-slate-200 text-sm font-medium hover:text-green-600 dark:hover:text-green-400 transition-colors duration-200"
            >
              About
            </Link>
